...
<!-- COMPONENT: PersonPopup { id: "karpathy" } -->Karpathy<!-- /COMPONENT --> added a framing that stuck: LLMs are a new kind of operating system. The context window is RAM. And context engineering is the discipline of managing that RAM — deciding what goes in, when, and in what order.
...
**Rules** are persistent behavioral constraints. They shape how the model operates without being explicitly invoked. Some are always active; others are gated by file patterns — a rule about Cloudflare configuration might activate only when you open a `wrangler.jsonc` file, lying dormant the rest of the time.

**Hooks** are event handlers that fire on system lifecycle events. They can observe (logging what happened after a tool call) or guard (blocking a dangerous command before it executes). Claude Code supports 22 lifecycle events including `PreToolUse`, `PostToolUse`, `SessionStart`, and `InstructionsLoaded`.

**Memory** is persistent state that survives across sessions. It's what makes the model remember your preferences, your project's conventions, and the decisions you've made in past conversations — without you repeating them every time.

**Tools** are external capabilities accessed via protocols like MCP (Model Context Protocol). They extend what the model can do beyond text: searching academic papers, scraping web pages, querying databases, running code. MCP is now supported by every major AI coding tool.

**References** are static documents loaded on demand. Templates, architecture decision records, style guides — content that isn't always in context but can be pulled in when relevant.
...
These six component types don't all load at once. They flow through a temporal pipeline with four stages — and the ordering matters.

**Build-time**: Plugins are installed. MCP servers are registered. Git hooks are wired up. Nothing enters the context window yet, but the infrastructure is in place.

**Session-start**: When a conversation begins, the system assembles the initial context. CLAUDE.md files are loaded (with a three-tier hierarchy: managed policy > project > user). Rules are discovered. Memory is recalled. MCP servers negotiate their capabilities. Skill metadata is scanned. The system prompt — 110+ modular segments in Claude Code — is assembled in a specific order optimized for cache efficiency.

**On-demand**: During conversation, components load in response to actions. A user invokes a skill. The model calls a tool. A path-scoped rule activates because a matching file was opened. A memory topic file is loaded because the memory agent decided it was relevant.

**Event-triggered**: Hooks fire on lifecycle events. System reminders inject context changes. When the context window approaches its limit, compaction kicks in — compressing the conversation while re-reading CLAUDE.md and memory fresh from disk.

The precedence rules are explicit. Components load at different scopes:

<!-- COMPONENT: CodeBlock { lang: "text", collapsible: true, title: "Context loading scopes (highest to lowest priority)" } -->
...
And you can observe the pipeline in action:

<!-- COMPONENT: CLISnippet { collapsible: true, title: "The InstructionsLoaded hook reveals what loaded and why" } -->
...
Post 2 in this series maps the full pipeline with implementation evidence and cross-platform validation.

<!-- COMPONENT: OGCard { url: "https://code.claude.com/docs/en/hooks" } -->
Claude Code Hooks Reference — 22 lifecycle events, handler types, and the InstructionsLoaded load_reason values that reveal context loading behavior.
<!-- /COMPONENT -->

But this isn't just a Claude Code thing. Every major AI coding tool has converged on similar architecture.
...
The six component types — skills, rules, hooks, memory, tools, references — are the building blocks of an agent harness. You're not building prompts. You're building a harness.
